{{-- CREATE MODAL --}}
<div id="createLaporanModal" class="fixed inset-0 bg-gray-600 bg-opacity-50 z-50 hidden overflow-y-auto">
    <div class="relative top-20 mx-auto p-6 w-11/12 md:w-3/4 lg:w-1/2 max-h-[90vh] overflow-y-auto">
        <div class="bg-white dark:bg-slate-800 rounded-3xl shadow-2xl">
            <div class="p-8 border-b border-slate-200 dark:border-slate-700 flex justify-between items-center relative">
                <h3 class="text-2xl font-bold text-slate-900 dark:text-white">Tambah Laporan HIMANIKKA</h3>
            ...
            <form id="createLaporanForm" enctype="multipart/form-data" class="p-8 space-y-6">
                {{-- Error Container --}}
                <div id="createErrors" class="space-y-2 hidden"></div>

                <!-- nama laporan -->
                <div>
                    <label class="block text-sm font-bold text-slate-700 dark:text-slate-300 mb-3 required">Nama Laporan *</label>
                ...
                <!-- tipe Laporan -->
                <div class="relative">
                    <label class="block text-sm font-bold text-slate-700 dark:text-slate-300 mb-3 required">Tipe Laporan *</label>
                ...
                <!-- upload File Laporan docx, pdf -->
                <div>
                    <label class="block text-sm font-bold text-slate-700 dark:text-slate-300 mb-3 required">File Laporan (PDF atau DOCX) *</label>
                ...
                <!-- status Laporan -->
                <div class="relative">
                    <label class="block text-sm font-bold text-slate-700 dark:text-slate-300 mb-3 required">Status Laporan *</label>
...
{{-- EDIT MODAL --}}
<div id="editLaporanModal" class="fixed inset-0 bg-gray-600 bg-opacity-50 z-50 hidden overflow-y-auto">
    <div class="relative top-20 mx-auto p-6 w-11/12 md:w-3/4 lg:w-1/2 max-h-[90vh] overflow-y-auto">
        <div class="bg-white dark:bg-slate-800 rounded-3xl shadow-2xl">
            <div class="p-8 border-b border-slate-200 dark:border-slate-700 flex justify-between items-center relative">
                <h3 class="text-2xl font-bold text-slate-900 dark:text-white">Edit Laporan</h3>
            ...
            <form id="editLaporanForm" enctype="multipart/form-data" class="p-8 space-y-6">
                <input type="hidden" name="id" id="editLaporanId">
                {{-- Error Container --}}
                ...
                {{-- File Laporan --}}
                <div>
                    <label class="block text-sm font-bold text-slate-700 dark:text-slate-300 mb-3">File Laporan (PDF atau DOCX)</label>
...
{{-- DELETE MODAL --}}
<div id="deleteLaporanModal" class="fixed inset-0 bg-gray-600 bg-opacity-50 z-50 hidden flex items-center justify-center p-4">
    <div class="bg-white dark:bg-slate-800 rounded-3xl shadow-2xl max-w-md w-full mx-4">
        <div class="p-8 text-center">
            ...
            <h3 class="text-lg font-bold text-slate-900 dark:text-white mb-2">Hapus Laporan?</h3>
            <p class="text-sm text-slate-600 dark:text-slate-400 mb-6" id="deleteLaporanName">Apakah Anda yakin ingin menghapus Laporan ini?</p>
            <input type="hidden" id="deleteLaporanId">
...
(function() {
    ...
    function refreshData() {
        if (window.Alpine && window.LaporanApp) {
            window.LaporanApp.refreshData?.();
    ...
    window.openCreateModal = function() {
        document.getElementById('createLaporanModal')?.classList.remove('hidden');
        hideErrors('createErrors');
        document.getElementById('createLaporanForm').reset();
    };

    window.closeCreateModal = function() {
        const modal = document.getElementById('createLaporanModal');
    ...
    window.openEditModal = function(id, laporanData) {
        const modal = document.getElementById('editLaporanModal');
        document.getElementById('editLaporanId').value = id;
        document.getElementById('editName').value = laporanData.name || '';
        document.getElementById('editType').value = laporanData.type || '';
        document.getElementById('editStatus').value = laporanData.status || '';
        
        // Show current file info
        const fileInfo = document.getElementById('currentFileInfo');
        const fileNameEl = document.getElementById('currentFileName');
        if (laporanData.file_name) {
            fileNameEl.textContent = laporanData.file_name;
    ...
    window.openEditModalFromRow = function(button) {
        ...
        const id = row.dataset.laporanId;
        let laporanData;
        
        try {
            laporanData = JSON.parse(row.dataset.laporanData || '{}');
        } catch (e) {
            console.error('Error parsing laporan data:', e);
            showNotification('Data laporan tidak valid', 'error');
            return;
        }
        
        openEditModal(id, laporanData);
    };

    window.openDeleteModal = function(id, name) {
        document.getElementById('deleteLaporanId').value = id;
        document.getElementById('deleteLaporanName').textContent = `Apakah Anda yakin ingin menghapus Laporan "${name}"?`;
    ...
    document.addEventListener('DOMContentLoaded', function() {
        // Create form
        const createForm = document.getElementById('createLaporanForm');
        if (createForm) {
        ...
        if (editForm) {
            editForm.addEventListener('submit', async (e) => {
                ...
                try {
                    const response = await fetch(`{{ route('admin.laporan.update', ':id') }}`.replace(':id', id), {
                        method: 'POST',
                        body: formData,
                        headers: {
                            'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]')?.getAttribute('content'),
        ...
        if (deleteBtn) {
            deleteBtn.addEventListener('click', async () => {
                ...
                try {
                    const response = await fetch(`{{ route('admin.laporan.destroy', ':id') }}`.replace(':id', id), {
                        method: 'DELETE',
                        headers: {
                            'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]')?.getAttribute('content'),
                    ...
                    if (response.ok) {
                        showNotification(result.message || 'Laporan berhasil dihapus!', 'success');
                        closeDeleteModal();
                        refreshData();
                    } else {
        ...
        document.querySelectorAll('#createLaporanModal, #editLaporanModal, #deleteLaporanModal').forEach(modal => {
