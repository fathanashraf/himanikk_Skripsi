...
<div id="deleteModal" class="fixed inset-0 bg-gray-600 bg-opacity-50 z-50 hidden flex items-center justify-center p-6">
    <div class="bg-white dark:bg-slate-800 rounded-3xl shadow-2xl max-w-md w-full mx-4 max-h-[90vh] overflow-y-auto">
        <div class="p-8 text-center border-b border-slate-200 dark:border-slate-700">
            <div class="w-20 h-20 bg-red-100 dark:bg-red-900/50 rounded-3xl flex items-center justify-center mx-auto mb-6">
                <svg class="w-12 h-12 text-red-600 dark:text-red-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z"/>
                </svg>
            </div>
            <h3 class="text-2xl font-bold text-slate-900 dark:text-white mb-4">Hapus Profil?</h3>
            <p class="text-slate-600 dark:text-slate-400 mb-8" id="deleteName">Apakah Anda yakin ingin menghapus profil ini?</p>
        </div>
        <form id="deleteForm">
            <input type="hidden" name="id" id="deleteId">
            <div class="p-8 pt-0 flex gap-3">
                <button type="submit" class="flex-1 bg-red-600 text-white font-bold py-4 px-6 rounded-xl hover:bg-red-700 transition-all duration-300 shadow-lg hover:shadow-xl hover:-translate-y-1">
                    Hapus
                </button>
                <button type="button" onclick="closeDeleteModal()" class="px-6 py-4 bg-slate-200 dark:bg-slate-700 text-slate-800 dark:text-slate-200 font-bold rounded-xl hover:bg-slate-300 dark:hover:bg-slate-600 transition-all duration-300">
                    Batal
                </button>
            </div>
        </form>
