{{-- CREATE MODAL --}}
<div id="createKegiatanModal" class="fixed inset-0 bg-gray-600 bg-opacity-50 z-50 hidden overflow-y-auto">
    <div class="relative top-20 mx-auto p-6 w-11/12 md:w-3/4 lg:w-1/2 max-h-[90vh] overflow-y-auto">
        <div class="bg-white dark:bg-slate-800 rounded-3xl shadow-2xl">
            <div class="p-8 border-b border-slate-200 dark:border-slate-700 flex justify-between items-center relative">
                <h3 class="text-2xl font-bold text-slate-900 dark:text-white">Tambah Kegiatan HIMANIKKA</h3>
            ...
            <form id="createKegiatanForm" enctype="multipart/form-data" class="p-8 space-y-6">
                {{-- Error Container --}}
                <div id="createErrors" class="space-y-2 hidden"></div>

                {{-- Nama Kegiatan --}}
                <div>
                    <label class="block text-sm font-bold text-slate-700 dark:text-slate-300 mb-3 required">Nama Kegiatan *</label>
                ...
                {{-- Gambar --}}
                <div>
                    <label class="block text-sm font-bold text-slate-700 dark:text-slate-300 mb-3">Gambar (Opsional)</label>
                    ...
                    <p class="mt-2 text-xs text-slate-500 dark:text-slate-400">Maksimal 5MB. Format JPG, PNG, WebP</p>
                </div>
                ...
                <div>
                    ...
                    <p class="mt-1 text-xs text-slate-500 dark:text-slate-400">Link ke website kegiatan atau pendaftaran</p>
                </div>
...
{{-- EDIT MODAL --}}
<div id="editKegiatanModal" class="fixed inset-0 bg-gray-600 bg-opacity-50 z-50 hidden overflow-y-auto">
    <div class="relative top-20 mx-auto p-6 w-11/12 md:w-3/4 lg:w-1/2 max-h-[90vh] overflow-y-auto">
        <div class="bg-white dark:bg-slate-800 rounded-3xl shadow-2xl">
            <div class="p-8 border-b border-slate-200 dark:border-slate-700 flex justify-between items-center relative">
                <h3 class="text-2xl font-bold text-slate-900 dark:text-white">Edit Kegiatan</h3>
            ...
            <form id="editKegiatanForm" enctype="multipart/form-data" class="p-8 space-y-6">
                ...
                {{-- Gambar --}}
                <div>
                    <label class="block text-sm font-bold text-slate-700 dark:text-slate-300 mb-3">Gambar</label>
...
<div id="deleteKegiatanModal" class="fixed inset-0 bg-gray-600 bg-opacity-50 z-50 hidden flex items-center justify-center p-4">
    <div class="bg-white dark:bg-slate-800 rounded-3xl shadow-2xl max-w-md w-full mx-4">
        <div class="p-8 text-center">
            ...
            <h3 class="text-lg font-bold text-slate-900 dark:text-white mb-2">Hapus Kegiatan?</h3>
            <p class="text-sm text-slate-600 dark:text-slate-400 mb-6" id="deleteKegiatanName">Apakah Anda yakin ingin menghapus kegiatan ini?</p>
            <input type="hidden" id="deleteKegiatanId">
...
(function() {
    ...
    function refreshData() {
        if (window.Alpine && window.KegiatanApp) {
            window.KegiatanApp.refreshData?.();
    ...
    window.openCreateModal = function() {
        document.getElementById('createKegiatanModal')?.classList.remove('hidden');
        hideErrors('createErrors');
        document.getElementById('createKegiatanForm').reset();
    };

    window.closeCreateModal = function() {
        const modal = document.getElementById('createKegiatanModal');
    ...
    window.closeEditModal = function() {
        document.getElementById('editKegiatanModal')?.classList.add('hidden');
    ...
    window.openEditModalFromRow = function(button) {
        ...
        const id = row.dataset.kegiatanId;
        let kegiatanData;
        
        try {
            kegiatanData = JSON.parse(row.dataset.kegiatanData || '{}');
        } catch (e) {
            console.error('Error parsing kegiatan data:', e);
            showNotification('Data kegiatan tidak valid', 'error');
            return;
        }
        
        openEditModal(id, kegiatanData);
    };

    window.openDeleteModal = function(id, name) {
        document.getElementById('deleteKegiatanId').value = id;
        document.getElementById('deleteKegiatanName').textContent = `Apakah Anda yakin ingin menghapus kegiatan "${name}"?`;
    ...
    document.addEventListener('DOMContentLoaded', function() {
        // Create form
        const createForm = document.getElementById('createKegiatanForm');
        if (createForm) {
            createForm.addEventListener('submit', async (e) => {
                ...
                try {
                    const response = await fetch('{{ route("admin.kegiatan.store") }}', {
                        method: 'POST',
                        body: formData,
                        headers: {
        ...
        if (editForm) {
            editForm.addEventListener('submit', async (e) => {
                ...
                try {
                    const response = await fetch(`{{ route('admin.kegiatan.update', ':id') }}`.replace(':id', id), {
                        method: 'POST',
                        body: formData,
                        headers: {
                            'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]')?.getAttribute('content'),
        ...
        if (deleteBtn) {
            deleteBtn.addEventListener('click', async () => {
                const id = document.getElementById('deleteKegiatanId').value;
                ...
                try {
                    const response = await fetch(`{{ route('admin.kegiatan.destroy', ':id') }}`.replace(':id', id), {
                        method: 'DELETE',
                        headers: {
                            'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]')?.getAttribute('content'),
                    ...
                    if (response.ok) {
                        showNotification(result.message || 'Kegiatan berhasil dihapus!', 'success');
                        closeDeleteModal();
                        refreshData();
                    } else {
        ...
        document.querySelectorAll('#createKegiatanModal, #editKegiatanModal, #deleteKegiatanModal').forEach(modal => {
