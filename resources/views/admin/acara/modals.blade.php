{{-- CREATE MODAL --}}
<div id="createAcaraModal" class="fixed inset-0 bg-gray-600 bg-opacity-50 z-50 hidden overflow-y-auto">
    <div class="relative top-20 mx-auto p-6 w-11/12 md:w-3/4 lg:w-1/2 max-h-[90vh] overflow-y-auto">
        <div class="bg-white dark:bg-slate-800 rounded-3xl shadow-2xl">
            <div class="p-8 border-b border-slate-200 dark:border-slate-700 flex justify-between items-center relative">
                <h3 class="text-2xl font-bold text-slate-900 dark:text-white">Tambah Acara HIMANIKKA</h3>
            ...
            <form id="createAcaraForm" enctype="multipart/form-data" class="p-8 space-y-6">
                {{-- Error Container --}}
...
{{-- EDIT MODAL --}}
<div id="editAcaraModal" class="fixed inset-0 bg-gray-600 bg-opacity-50 z-50 hidden overflow-y-auto">
    <div class="relative top-20 mx-auto p-6 w-11/12 md:w-3/4 lg:w-1/2 max-h-[90vh] overflow-y-auto">
        <div class="bg-white dark:bg-slate-800 rounded-3xl shadow-2xl">
            <div class="p-8 border-b border-slate-200 dark:border-slate-700 flex justify-between items-center relative">
                <h3 class="text-2xl font-bold text-slate-900 dark:text-white">Edit Acara</h3>
            ...
            <form id="editAcaraForm" enctype="multipart/form-data" class="p-8 space-y-6">
                <input type="hidden" name="id" id="editAcaraId">
                {{-- Error Container --}}
...
(function() {
    ...
    function refreshData() {
        if (window.Alpine && window.AcaraApp) {
            window.AcaraApp.refreshData?.();
    ...
    window.openCreateModal = function() {
        document.getElementById('createAcaraModal')?.classList.remove('hidden');
        hideErrors('createErrors');
        document.getElementById('createAcaraForm').reset();
    };

    window.closeCreateModal = function() {
        const modal = document.getElementById('createAcaraModal');
        const form = document.getElementById('createAcaraForm');
    ...
    window.closeEditModal = function() {
        document.getElementById('editAcaraModal')?.classList.add('hidden');
    ...
    window.closeDeleteModal = function() {
        document.getElementById('deleteAcaraModal')?.classList.add('hidden');
    };

    // === FORM SUBMISSIONS ===
    document.addEventListener('DOMContentLoaded', function() {
        // Create form
        const createForm = document.getElementById('createAcaraForm');
        if (createForm) {
            createForm.addEventListener('submit', async (e) => {
                ...
                try {
                    const response = await fetch('{{ route("admin.acara.store") }}', {
                        method: 'POST',
                        body: formData,
                        headers: {
        ...
        // Edit form
        const editForm = document.getElementById('editAcaraForm');
        if (editForm) {
            editForm.addEventListener('submit', async (e) => {
                e.preventDefault();
                const id = document.getElementById('editAcaraId').value;
                ...
                try {
                    const response = await fetch(`{{ route('admin.acara.update', ':id') }}`.replace(':id', id), {
                        method: 'POST',
                        body: formData,
                        headers: {
                            'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]')?.getAttribute('content'),
        ...
        if (deleteBtn) {
            deleteBtn.addEventListener('click', async () => {
                const id = document.getElementById('deleteAcaraId').value;
                ...
                try {
                    const response = await fetch(`{{ route('admin.acara.destroy', ':id') }}`.replace(':id', id), {
                        method: 'DELETE',
                        headers: {
                            'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]')?.getAttribute('content'),
                    ...
                    if (response.ok) {
                        showNotification(result.message || 'Acara berhasil dihapus!', 'success');
                        closeDeleteModal();
                        refreshData();
                    } else {
        ...
        document.querySelectorAll('#createAcaraModal, #editAcaraModal, #deleteAcaraModal').forEach(modal => {
