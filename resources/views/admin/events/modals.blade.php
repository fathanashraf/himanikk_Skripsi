        ...
        <div class="bg-white dark:bg-slate-800 rounded-3xl shadow-2xl">
            ...
            <form id="createEventForm" enctype="multipart/form-data" class="p-8 space-y-6">
                ...
                <div>
                    ...
                    <p class="mt-1 text-xs text-slate-500 dark:text-slate-400">Link ke website event atau pendaftaran</p>
                </div>
...
(function() {
    ...
    window.openEditModalFromRow = function(button) {
        ...
        try {
            eventData = JSON.parse(row.dataset.eventData || '{}');
        } catch (e) {
            console.error('Error parsing event data:', e);
            showNotification('Data event tidak valid', 'error');
            return;
        }
        
        openEditModal(id, eventData);
    };
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        if (editForm) {
            editForm.addEventListener('submit', async (e) => {
                ...
                try {
                    const response = await fetch(`{{ route('admin.events.update', ':id') }}`.replace(':id', id), {
                        method: 'POST',
                        body: formData,
                        headers: {
                            'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]')?.getAttribute('content'),
        ...
        if (deleteBtn) {
            deleteBtn.addEventListener('click', async () => {
                ...
                try {
                    const response = await fetch(`{{ route('admin.events.destroy', ':id') }}`.replace(':id', id), {
                        method: 'DELETE',
                        headers: {
                            'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]')?.getAttribute('content'),
